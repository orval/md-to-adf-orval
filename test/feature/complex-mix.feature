Feature: Complex formatting mix
  ...
  Scenario: Headers, List, emphasis, smiley, inline code, empty lines and code block
    ...
    And the ADF chunk at content path [ 1, 1 ] has type 'listItem'
    And the ADF chunk at content path [ 1, 1, 0 ] has type 'paragraph'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": "Ability to define a comparator function for packets " }'
    And the ADF chunk at content path [ 1, 1, 0, 1 ] has type 'emoji'
    And the ADF chunk at content path [ 1, 1, 0, 1 ] has attribute '{ "shortName": "+1" }'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": " these correspond to two abilities in the " }'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": "editcap", "marks": [ { "type": "code" } ] }'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": " tool from Wireshark. For example: " }'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": "editcap -D 100 -I 26 myfile.pcap", "marks": [ { "type": "code" } ] }'
    And the ADF chunk at content path [ 1, 1, 0 ] contains '{"type": "text", "text": " where:" }'
    And the ADF chunk at content path [ 1, 1, 1 ] has type 'codeBlock'
    And the ADF chunk at content path [ 1, 1, 1 ] contains '{"type": "text", "text": "-I <bytes to ignore>   ignore the specified number of bytes at the beginning\nof the frame during MD5 hash calculation, unless the\nframe is too short, then the full frame is used.\n\n\n-D <dup window>        remove packet if duplicate; configurable <dup window>.\nValid <dup window> values are 0 to 1000000.\n" }'
    And the ADF chunk at content path [ 1, 1, 2 ] has type 'paragraph'
    And the ADF chunk at content path [ 1, 1, 2 ] contains '{"type": "text", "text": "-D", "marks": [ { "type": "code" } ] }'
    And the ADF chunk at content path [ 1, 1, 2 ] contains '{"type": "text", "text": " and " }'
    And the ADF chunk at content path [ 1, 1, 2 ] contains '{"type": "text", "text": "-w", "marks": [ { "type": "code" } ] }'
    And the ADF chunk at content path [ 1, 1, 2 ] contains '{"type": "text", "text": " are ways to group duplicates." }'
    And the ADF chunk at content path [ 1, 1, 3 ] has type 'codeBlock'
    And the ADF chunk at content path [ 1, 1, 3 ] contains '{"type": "text", "text": "compare(p1: Node, p2: Node) = {\nexcludeTags(name: String, node: Node) = (node \\ \"@name\").text == name\np1.filter(excludeTags(\"ip.src\")) == p2.filter(excludeTags(\"ip.src\"))\n} // must return Boolean" }'
    And the ADF chunk at content path [ 2 ] has type 'heading'
    And the ADF chunk at content path [ 2 ] contains '{"type": "text", "text": "This "}'
    And the ADF chunk at content path [ 2 ] contains '{"type": "text", "text": "is ", "marks": [ { "type": "em" } ] }'
    And the ADF chunk at content path [ 2 ] contains '{"type": "text", "text": "a second", "marks": [ { "type": "strong" }, { "type": "em" } ] }'
    And the ADF chunk at content path [ 3 ] has type 'paragraph'
    And the ADF chunk at content path [ 3 ] contains '{"type": "text", "text": "And a paragraph and more text inside the same paragraph" }'
